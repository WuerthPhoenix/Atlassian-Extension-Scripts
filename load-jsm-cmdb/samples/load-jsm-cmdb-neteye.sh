...
loc=$(dirname "$0")
me=`basename "$0"`
source $loc/jira_variables_neteye.sh $me $loc
...
createJson4products(){
     ...
     #numberOfAttr=$(echo $numberOfAttr|  sed 's/\"//g' )
...
###################################
 ...
 jq ' .[]  |  "Name:"+.PRODUCTNAME+";"                            ,
       ...
       "Price:"+.PRICE+";"                                        ,
       "SearchKey:"+.SEARCHKEY+";"                                ,
       "Link:"+.LINK+"|"  ' "$productdatajsonfile" |  while read -r -d "|" productline; do


                Productresult=$(createJson4products "$productline" "neteyehosts" )  # TODO - Improvement - The Type can be retrieved from jira.conf
                echo $(createInsightObject "$Productresult" "neteyehosts" )
